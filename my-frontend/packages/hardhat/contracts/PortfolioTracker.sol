...
contract PortfolioTracker is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    event ManualPriceSet(uint8 indexed asset, uint256 priceUsdScaled, bool enabled);
    event PositionSet(
        uint8 indexed asset,
    ...
    event ClassCreated(uint256 indexed classId, uint256 indexed parentId, string name);
    ...
    event SellRecorded(uint8 indexed asset, uint256 amountScaled, uint256 priceUsdScaled, int256 realizedPnlDelta);
    ...
    // -----------------------
    function createClass(string calldata name, uint256 parentId) external onlyOwner returns (uint256 classId) {
        return _createClassInternal(name, parentId);
    }

    function _createClassInternal(string memory name, uint256 parentId) internal returns (uint256 classId) {
        require(bytes(name).length > 0, "name empty");
        if (parentId != 0) {
            require(_classes[parentId].exists && _classes[parentId].active, "bad parent");
        }

        classId = nextClassId;
        nextClassId++;

        _classes[classId] = ClassInfo({ exists: true, active: true, parentId: parentId, name: name });
    ...
    function renameClass(uint256 classId, string calldata name) external onlyOwner {
        require(_classes[classId].exists, "no class");
        require(bytes(name).length > 0, "name empty");
    ...
    function getClassInfo(
        uint256 classId
    ) external view returns (bool exists, bool active, uint256 parentId, string memory name) {
    ...
    function setAssetClass(AssetId asset, uint256 classId) external onlyOwner {
        require(_classes[classId].exists && _classes[classId].active, "bad class");
    ...
    // -----------------------
    function recordSell(AssetId asset, uint256 sellAmountScaled, uint256 sellPriceUsdScaled) external {
        ...
        uint256 principalSold = sellAmountScaled <= p.amount ? sellAmountScaled : p.amount;
        ...
        if (principalSold > 0) {
            ...
            int256 priceDiff = int256(sellPriceUsdScaled) - int256(p.avgBuyPrice);
        ...
        if (rewardsSold > 0) {
            ...
            pnlDelta += (int256(sellPriceUsdScaled) * int256(rewardsSold)) / int256(SCALE);
        ...
        emit SellRecorded(uint8(asset), sellAmountScaled, sellPriceUsdScaled, pnlDelta);
    }
    ...
    function setManualPrice(AssetId asset, uint256 priceUsdScaled, bool enabled) external onlyOwner {
    ...
    function setPosition(
        ...
        positions[asset] = Position(amountScaled, avgBuyPriceScaled, stakingRewardsScaled);
        emit PositionSet(uint8(asset), amountScaled, avgBuyPriceScaled, stakingRewardsScaled);
    }

    function addStakingRewards(AssetId asset, uint256 extraRewardsScaled) external onlyOwner {
    ...
    function safeGetPriceUsdScaled(AssetId asset) public view returns (uint256) {
    ...
    function getAssetReport(AssetId asset) public view returns (AssetReport memory) {
